...
# 監聽的資料夾路徑
folder_path = '/home/beta/fridgeweb/raspiphoto/final'  # 根據您的實際路徑更改
alarm_time = '/home/beta/fridgeweb/raspiphoto/toSQL/alarm_time.json'
name_table = '/home/beta/fridgeweb/raspiphoto/toSQL/name_table.json'
photos = '/home/beta/fridgeweb/public/photos'
wrongphoto = '/home/beta/fridgeweb/wrongphoto'

print("=========================")
print("*** Start AddToSQL.py ***")
print("=========================")

while True:
    # 獲取資料夾中的所有檔案
    files = os.listdir(folder_path)
    print("AddToSQL.py: Start round.")
    ...
    if files:
        ...
        # 輸出偵錯訊息：獲取照片名稱
        print(f'AddToSQL.py: Get photo "{file_name}"')
        ...
        if len(file_name_parts) == 5:
            ...
            if in_out == 1:
                ...
                try:
                    with open(alarm_time, 'r') as json_file:
                        alarm_data = json.load(json_file)
                        duration_days = int(alarm_data.get(kind, -1))  # 將 '7' 解釋為整數 7
                        if duration_days == -1:
                            print(f'AddToSQL.py: !Error! Without expiry date data in json: "{kind}"')
                            duration_days = 7
                except FileNotFoundError:
                    print('AddToSQL.py: !Error! No exist alarm_time.json.')
                    duration_days = 7
                ...
                try:
                    with open(name_table, 'r') as name_file:
                        name_data = json.load(name_file)
                        CTname = name_data.get(kind, '--')
                        if CTname == '--':
                            print(f'AddToSQL.py: !Error! Without name data in json: "{kind}"')
                            CTname = kind
                except FileNotFoundError:
                    print('AddToSQL.py: !Error! No exist nameTable.json.')
                ...
                connection.commit()
                print(f'AddToSQL.py: Updated product with name "{CTname}", kind "{kind}" in fridge {fridge_id} to Not Exist.')
            ...
            # 輸出偵錯訊息：移動檔案
            print(f'AddToSQL.py: Moving file: "{file_name}"')

        else:
            # 檔案名稱不符合規則，輸出錯誤訊息
            print(f'AddToSQL.py: !Error! Wrong file format or Wrong name format: "{file_name}"')
    ...
    # print(f'AddToSQL.py: Waiting for new photos...\n')
    time.sleep(5)
